...
def addNote():
    ...
    with tempfile.NamedTemporaryFile(suffix=".tmp") as tf:
        ...
        with open(bookFolder + book, 'a') as the_file:
...
def delBook():
    ...
    rm = notebook[delBook]
    call(["rm", bookFolder + rm])
    del notebook[delBook]
    print(delBook)
    with open(bookConfig, "w") as f:
        json.dump(notebook, f)
    clearScreen()
    print(Fore.RED + "INFO: " + Fore.CYAN + "Book " + Fore.RED + str(rm) + Fore.CYAN + " has been deleted" + Fore.WHITE)
...
def editNote():
    with open(bookConfig) as f:
        notebook = json.load(f)
    header()
    print(Fore.CYAN + "-- Which book to edit? --")
    # Prints out list of notepads
    print(Fore.YELLOW)
    for i, item in enumerate(notebook, 0):
        print(i, ": " + item, sep="", end="\n")
